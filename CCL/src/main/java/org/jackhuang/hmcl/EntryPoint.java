...
public final class EntryPoint {
    ...
    private static void checkSingleInstance() {
        Path lockFilePath = Metadata.HMCL_GLOBAL_DIRECTORY.resolve("hmcl.lock");
        try {
            ...
            if (lock == null) {
                // 锁被占用，提示并退出
                showErrorAndExit(i18n("fatal.instance_already_running"));
            }
            ...
            Runtime.getRuntime().addShutdownHook(new Thread(() -> {
                try {
                    if (lock != null) {
                        ...
                        channel.close();
                        lockFile.close();
                        Files.deleteIfExists(lockFilePath); // 可选：清理锁文件
                    }
                } catch (IOException e) {
                    LOG.error("释放锁文件失败", e);
                }
            ...
            showErrorAndExit(i18n("fatal.lock_file_error", lockFilePath));
        }
    }
