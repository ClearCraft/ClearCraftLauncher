        ...
        public AccountListPageSkin(AccountListPage skinnable) {
...

            {
                VBox boxMethods = new VBox();
                {
                    ...
                    AdvancedListItem ClearCraftItem = new AdvancedListItem();
                    ClearCraftItem.getStyleClass().add("navigation-drawer-item");
                    ClearCraftItem.setActionButtonVisible(false);
                    ClearCraftItem.setTitle("ClearCraft");
                    ClearCraftItem.setLeftGraphic(wrap(SVG.PERSON));
                    ClearCraftItem.setOnAction(e -> Controllers.dialog(new CreateAccountPane(new AuthlibInjectorServer("https://skins.clearcraft.cn/api/yggdrasil/"))));

                    AdvancedListItem RainCraftItem = new AdvancedListItem();
                    RainCraftItem.getStyleClass().add("navigation-drawer-item");
                    RainCraftItem.setTitle("RainCraft");
                    RainCraftItem.setActionButtonVisible(false);
                    RainCraftItem.setLeftGraphic(wrap(SVG.PERSON));
                    ...
                    if (RESTRICTED.get()) {
                        VBox wrapper = new VBox(ClearCraftItem, RainCraftItem);
                        wrapper.setPadding(Insets.EMPTY);
                        FXUtils.installFastTooltip(wrapper, i18n("account.login.restricted"));

                        ClearCraftItem.setDisable(true);
                        RainCraftItem.setDisable(true);
                        ...
                        holder = FXUtils.onWeakChange(RESTRICTED, value -> {
                            if (!value) {
                                holder = null;
                                ClearCraftItem.setDisable(false);
                                RainCraftItem.setDisable(false);
                                boxAuthServers.setDisable(false);
                                boxMethods.getChildren().setAll(title, microsoftItem, ClearCraftItem, RainCraftItem);
                            }
                        });
                    } else {
                        boxMethods.getChildren().setAll(title, microsoftItem, ClearCraftItem, RainCraftItem, boxAuthServers);
                    }
                }
