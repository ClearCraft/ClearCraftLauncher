...
import org.jackhuang.hmcl.util.LocalHttpServer;
...
public class RootPage extends DecoratorAnimatedPage implements DecoratorPage {
    ...
    public RootPage() {
        ...
        while (true) {
            try {
                LocalHttpServer.getInstance().start(port);

                if (LocalHttpServer.getInstance().isRunning()) {
                    LOG.info("Server is running");
                }
                break;
            }catch (IOException e) {
                if (port >= 1200) {
                    throw new RuntimeException("Failed to start local http server", e);
                }
                port++;
            }
        }
