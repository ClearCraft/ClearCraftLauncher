...
import static org.jackhuang.hmcl.ui.FXUtils.bind;
...
public class CreateAccountPane extends JFXDialogLayout implements DialogAware {
    ...
    public CreateAccountPane(AccountFactory<?> factory) {
...

        {
            ...
            } else if (factory instanceof BoundAuthlibInjectorAccountFactory) {  // 先进行类型检查
                AuthlibInjectorServer server = ((BoundAuthlibInjectorAccountFactory) factory).getServer();
                if ("https://skins.clearcraft.cn/api/yggdrasil/".equals(server.getUrl())) {
                    title = "account.create.clearcraft";
                } else if ("https://skin.ineko.cc/api/yggdrasil/".equals(server.getUrl())) {
                    title = "account.create.raincraft";
                } else {
    ...
    private static class AccountDetailsInputPane extends GridPane {
        ...
        private static List<Hyperlink> createHyperlinks(AuthlibInjectorServer server) {
            ...
            if (Objects.equals(server.getUrl(), "https://skins.clearcraft.cn/api/yggdrasil/") || Objects.equals(server.getUrl(), "https://skin.ineko.cc/api/yggdrasil/")) {
                for (String key : ALLOWED_LINKS) {
                    String keyValue = null;
                    if (key.equals("homepage"))
                        keyValue = "/";
                    if (key.equals("register"))
                        keyValue = "/auth/register";
                    String value = server.getUrl().replaceAll("/api/yggdrasil/$", "") + keyValue;
                    if (value != null) {
