...
public final class VersionSettingsPage extends StackPane implements DecoratorPage, VersionPage.VersionLoadable, PageAware {
    ...
    private final JFXComboBox<String> cboServerIP;
    ...
    private final Label lblPing = new Label();
    ...
    public VersionSettingsPage(boolean globalSetting) {
...

        {
            ...
            {
                Label label = new Label(i18n("settings.advanced.server_ip"));
                serverPane.setLeft(label);
                BorderPane.setAlignment(label, Pos.CENTER_LEFT);
                pingExecutor = Executors.newSingleThreadScheduledExecutor();
                cboServerIP = new JFXComboBox<>();
// 首先，设置下拉框为不可编辑（默认状态）
...
// 添加选择监听器
                ...
                serverPane.setRight(serverBox);
                BorderPane.setAlignment(serverBox, Pos.CENTER_RIGHT);
                FXUtils.setLimitWidth(serverBox, 300);
            }
    ...
    public void loadVersion(Profile profile, String versionId) {
        ...
        if (lastVersionSetting != null) {
            ...
            cboServerIP.valueProperty().unbindBidirectional(lastVersionSetting.serverIpProperty());
        ...
        cboServerIP.valueProperty().bindBidirectional(versionSetting.serverIpProperty());
